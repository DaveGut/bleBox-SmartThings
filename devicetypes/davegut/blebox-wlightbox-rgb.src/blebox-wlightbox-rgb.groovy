...
def on() {
	...
	parent.childCommand(getDataValue("channel"), state.savedRgb, state.transTime)
}

def off() {
	logDebug("off")
	parent.childCommand(getDataValue("channel"), "000000", state.transTime)
}

def setRed(red) {
	logDebug("setRed: ${red})")
	red = Integer.toHexString(red).padLeft(2, '0')
    def rgb = red + state.savedRgb[2..5]
	parent.childCommand(getDataValue("channel"), rgb, state.transTime)
}

def setGreen(green) {
	logDebug("setGreen: ${green})")
	green = Integer.toHexString(green).padLeft(2, '0')
    def rgb = state.savedRgb[0..1] + green + state.savedRgb[4..5]
	parent.childCommand(getDataValue("channel"), rgb, state.transTime)
}
...
def setBlue(blue) {
    ...
    def rgb = state.savedRgb[0..3] + blue
	parent.childCommand(getDataValue("channel"), rgb, state.transTime)
}
...
def parseReturnData(hexDesired) {
    ...
    hexDesired = hexDesired[0..5]
	if (hexDesired == "000000") {
		sendEvent(name: "switch", value: "off")
		logInfo("Device is off.")
	} else {
		sendEvent(name: "switch", value: "on")
		state.savedRgb = hexDesired
